...
static uint8_t _tx_buffer[sizeof(RingBuffer) + BUFLEN]
    __attribute__((aligned(4)));
static uint8_t _rx_buffer[sizeof(RingBuffer) + BUFLEN]
    __attribute__((aligned(4)));
...
void uart_write_hex(unsigned long int hex) {
  uint8_t ii;
  uint8_t jj;
  char towrite;
  uart_write("0x", 2);
...
int uart_write_err(char *p, int len) {
  ...
  for (i = 0; i < len; i++) {
    while ((UART0_S1 & UART_S1_TDRE_MASK) ==
           0) // Wait until transmit buffer empty
      ;
